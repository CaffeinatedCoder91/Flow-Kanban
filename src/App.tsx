  ...
  return (
    <div className="app">
      ...
      {importSuccessMessage && (
        <div className="toast-notification">{importSuccessMessage}</div>
      )}
      {toastMessage && (
      ...
      {errorToast && (
        <div className="toast-notification toast-error">
          ...
          {errorToast.onRetry && (
            <button className="toast-retry-btn" onClick={() => { setErrorToast(null); errorToast.onRetry!() }}>
              Try again
            </button>
          )}
        </div>
      ...
      {isImportOpen && (
        <div className="modal-overlay" onClick={() => { if (!isExtracting && !isConfirming) closeImportModal() }}>
          <div className={`modal${extractedTasks.length > 0 ? ' modal-wide' : ''}`} onClick={e => e.stopPropagation()}>
            <div className="modal-header">
              <h2>📋 Import Tasks</h2>
              <button className="modal-close" onClick={closeImportModal} aria-label="Close" disabled={isExtracting || isConfirming}>&times;</button>
            </div>

            {extractedTasks.length === 0 ? (
              <>
                <div className="modal-body">
                  <p className="modal-hint">Paste your tasks below, or upload a file.</p>
                  <textarea
                  ...
                  />
                  {extractError && <p className="modal-extract-error">{extractError}</p>}
                  {isExtracting && (
                    <div className="modal-spinner-row">
                      <span className="modal-spinner" />
                      <span className="modal-spinner-label">Analysing...</span>
                    </div>
                  )}
                </div>
                <div className="modal-footer">
                  <label className="modal-file-btn" aria-label="Upload file">
                  ...
                  <span className="modal-file-hint">.txt · .pdf · .docx · max 5MB</span>
                  <button className="modal-cancel" onClick={closeImportModal} disabled={isExtracting}>Cancel</button>
                  <button
                    className="modal-process"
              ...
              <>
                <div className="modal-body modal-preview-body">
                  {importFileName && (
                    <p className="modal-file-source">📄 Tasks from: <strong>{importFileName}</strong></p>
                  )}
                  <p className="modal-preview-count">
                ...
                <div className="modal-footer">
                  <button className="modal-cancel" onClick={() => { setExtractedTasks([]); setExtractError(null) }} disabled={isConfirming}>Back</button>
                  <button className="modal-process" onClick={handleConfirmAll} disabled={isConfirming || extractedTasks.length === 0}>
                    {isConfirming ? (
                      <><span className="modal-spinner modal-spinner-inline" /> Adding...</>
                    ) : (
                      `Confirm All (${extractedTasks.length})`
                    )}
                  </button>
                </div>
              </>
